...
st.image('https://i.imgur.com/MDKQoDc.jpg',width=500)
...
st.data_editor(df)
st.checkbox('Option 1') #여러 문항중 중복선택
st.radio('Pick Country:', ['France','Germany'])  #여러개중 단일항목 선택
st.selectbox('Select', [1,2,3]) #여러개중 단일 항목선택
st.multiselect('Multiselect', [1,2,3]) # 여러개중 중복항목선택
st.slider('Slide me', min_value=0, max_value=10)
st.select_slider('Slide to select', options=[1,'2',3])
st.text_input('Enter Article')
st.number_input('Enter required number') #많이쓰는거
st.text_area('Entered article text') # 텍스트박스랑 비슷한데 조금 더 큰 영역으로 받는 텍스트 박스
